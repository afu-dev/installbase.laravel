...
class ModbusParser extends AbstractRawDataParser
{
    protected function parseData(): array
    {
        ...
        foreach ($units as $unit) {
            ...
            foreach ($lines as $index => $line) {
                [$key, $value] = explode(':', preg_replace('/[^\x20-\x7E]/', '', str_replace("\n", " ", $line)), 2);
                if ($index === 0) {
                    $unitId = trim($value);
                }
    ...
    private function extractDeviceInformations(array $unitData): ?array
    {
        ...
        $identificationParts = explode(" ", $unitData["Device Identification"]);

        if (str_contains($unitData["Device Identification"], 'Schneider Electric')) {
